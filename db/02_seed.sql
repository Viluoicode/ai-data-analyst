/* ============================================================================
   ...
   No NEWID()/RAND() without a fixed seed: the dataset is identical on every
   load, so the evaluation golden-set expected results stay stable.
   Requires SQL Server 2022+ (uses GENERATE_SERIES).
   ============================================================================ */
...
/* -- DimCustomer (300, generated deterministically) -------------------------- */
;WITH c AS (
    SELECT value AS k FROM GENERATE_SERIES(1, 300)
)
...
SELECT
    ...
    CASE WHEN k % 10 = 0 THEN 'Gold'
         WHEN k % 10 IN (1,2,3) THEN 'Silver'
         ELSE 'Bronze' END,
    CASE k % 4 WHEN 0 THEN N'Hà Nội' WHEN 1 THEN N'Hồ Chí Minh'
               WHEN 2 THEN N'Đà Nẵng' ELSE N'Hải Phòng' END,
...
INSERT gold.DimDate (DateKey, FullDate, [Year], [Quarter], MonthNum, MonthName,
                     DayNum, DayOfWeekNum, DayName, IsWeekend)
SELECT
    ...
    DAY(FullDate),
    ((DATEPART(weekday, FullDate) + @@DATEFIRST - 2) % 7) + 1,          -- 1=Mon..7=Sun
    DATENAME(weekday, FullDate),
    CASE WHEN ((DATEPART(weekday, FullDate) + @@DATEFIRST - 2) % 7) + 1 IN (6,7)
         THEN 1 ELSE 0 END
FROM d;
GO

/* -- FactOrderItem (deterministic; 3 line items per order) ------------------- */
...
;WITH items AS (
    SELECT value AS n, ((value - 1) / 3) + 1 AS OrderId
...
ord AS (
    SELECT
        i.n,
        i.OrderId,
        1 + (i.OrderId * 5)  % @numStores    AS StoreKey,     -- mult 5 coprime with 6
        1 + (i.OrderId * 13) % @numCustomers AS CustomerKey,
        1 + (i.n * 7)        % @numProducts  AS ProductKey,   -- mult 7 coprime with 18
        1 + (i.n % 3)                        AS Quantity,
        (i.OrderId * 11) % @numDays          AS DayOffset,    -- mult 11 coprime with 731
        i.OrderId % 3                        AS PayIdx
    FROM items i
)
INSERT gold.FactOrderItem
    (OrderItemKey, OrderId, DateKey, StoreKey, ProductKey, CustomerKey,
     Quantity, UnitPrice, DiscountAmount, LineTotal, PaymentMethod)
SELECT
    o.n,
    o.OrderId,
...
PRINT CONCAT('Seed complete. FactOrderItem rows = ', @rows);
GO
